...
-------------------------------------------------
  ...
  一つの方法として、vsh.txt/game.txt/pops.txtに書かれているMacroFireの行を先頭の方へ移動した上で、
  macrofire.iniのStartupをOnにしてみてください。
...
-------------------------------------------------
  ...
  これは、このゲーム終了ダイアログが、MacroFireの扱っているところとは違う方法でボタンを検出しているからのようです。
  このようなボタンは、他にもリモコンの音量ボタンがあり、リモコンの音量ボタンを別のボタンに割り当てても
  そのボタンを押したときに音量が変わります。
  
  いくつかのプラグインでは、音量ボタンの再割り当てに成功しているようなので、
  方法はあるようなのですが、今のところ詳細はわかっていません。
